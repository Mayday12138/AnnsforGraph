...
class HNSW:
    def __init__(self, data, M, num_levels, ef_construction,queries,ground_truth):
        self.data = data
        self.M = M
        self.num_levels = num_levels
        self.ef_construction = ef_construction
        # 初始化每层的图结构，为每个数据点创建一个空列表
        self.graphs = [[[] for _ in range(len(data))] for _ in range(num_levels)]
    ...
    def build(self):
        ...
        for level in range(self.num_levels):
            ...
            for i, point in enumerate(self.data):
                self._connect_points(level, i, M_level)

    def build_graph_for_level(self, level, M_level):
        # 为特定层次构建图
        for i, point in enumerate(self.data):
            # 找到M_level个最近邻居
            # 注意：这里需要修改为找到M_level个最近邻居，而不是M+1
            neighbors_indices = self._find_neighbors(i, M_level, level)
            # 将邻居添加到当前层次的图中
            for neighbor_idx in neighbors_indices:
                self.graphs[level][i].append(neighbor_idx)

    def _connect_points(self, level, point_idx, M):
        # 使用最近邻算法找到 M 个最近的邻居
        nn = NearestNeighbors(n_neighbors=M + 1, metric='euclidean').fit(self.data)
        distances, indices = nn.kneighbors([self.data[point_idx]])
        distances = distances.flatten()
        indices = indices.flatten()

        # 选择最近的 M 个邻居，排除自身
        nearest_neighbors = indices[1:M + 1]

        # 将这些邻居添加到图中
        for neighbor_idx in nearest_neighbors:
            self.graphs[level][point_idx].append(neighbor_idx)
...
# 初始化 HNSW
M = 50  # 每层的出边数 有效测试（1）M=100，ef_search = 500，ef_construction = 550，numlevel=3
ef_construction = 450  # 图构建时的候选点数
ef_search = 10  # 搜索时考虑的候选点数

hnsw = HNSW(data,M,2,ef_construction,queries,ground_truth)
hnsw.data = data
start_time = time.time()
hnsw.build()
# 检查图是否为空
